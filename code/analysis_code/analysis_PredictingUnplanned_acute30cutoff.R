...
# (using acute care LOS as a binary predictor) 
#
...
# information in addition to demographic variables. We use a 30-day cutoff rule
# to change acute care LOS into a binary predictor.
# 
# Created by Isaac Chen 2024.05.08
#
# 2024.05.14 Added a section to check for post-test model assumptions
...
pre_full_mod_rms <- 
  lrm(
    Transfer_Status ~ Age_Adm + Acute_LOS_30cutoff + Sex + Etiology_TBIOther + 
...
# Reduced model
pre <- glm(
  Transfer_Status ~ Acute_LOS_30cutoff, 
...
post <- glm(
  Transfer_Status ~ Acute_LOS_30cutoff + Screening_Status_24hr, 
  data = CAPD_prog, family = "binomial"
)
post_rms <- lrm(
  Transfer_Status ~ Acute_LOS_30cutoff + Screening_Status_24hr, 
...
# Save figure
fig_path <- here(
  "results", "figures", "unplanned_transfer_acute30cutoff",
...
fig_scatter_prepost <- tbl_prepost|> 
  ...
  labs(
    title = "Pre- vs Post-test Risk (n = 113)",
    x = "Predicted Risk without CAPD", 
    y = "Predicted Risk with CAPD",
    col = ""
  ) + 
  scale_color_discrete(labels = c("Unplanned Transfers", "None/Planned")) +
...
fig_hist_prepost <- tbl_prepost|> 
  ...
  labs(
    title = "Pre- vs Post-test Risk (n = 113)",
    x = "Predicted Risk of Unplanned Transfer", 
    y = "Count"
  ) + 
  scale_x_continuous(breaks = seq(0, 1, 0.1)) +
  scale_y_continuous(breaks = seq(-60, 50, 5)) + 
  theme_bw() + 
  coord_flip()
fig_hist_prepost
